...
| Lifecycle state | `SCOPE_LOCKED`; G2-A, G2-B, and G2-C are complete and the remaining 1.0 work is deliberately bounded. |
| Current branch | `main` |
| Current decision | `TDE_1_0_SCOPE_LOCKED` |
...
| Next planned public release | `1.0.0`, after one immutable candidate is qualified with the selected consumer, artifacts, Runtime, and release evidence. See [TDE 1.0 Scope Lock](TDE_1_0_SCOPE_LOCK.md). |
...
| Next recommended engineering work | Create and qualify the immutable `1.0.0` candidate, then run the selected `djconnect-pi` consumer against that exact public candidate in OBSERVE mode. Do not promote to warn, soft fail, required, or wider rollout. |
