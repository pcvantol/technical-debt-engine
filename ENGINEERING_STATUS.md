...
| Lifecycle state | `RC1_RELEASE_CAPABILITY_REPAIR_REVIEWABLE_FROZEN`; G2-A, G2-B, and G2-C are complete and the remaining 1.0 work is deliberately bounded. |
| Current branch | `codex/rc1-release-capability-aliases` |
| Current decision | `NO_GO_TDE_1_0_0_RC1_RELEASE_CAPABILITY_REPAIR_PREPARED` |
...
| Next recommended engineering work | Review and merge the bounded release-capability selection repair in PR #130, then create a new immutable RC2 from the exact merged `main` SHA. The failed RC1 candidates remain immutable NO-GO evidence. Do not promote to warn, soft fail, required, or wider rollout. |
