...
| G2-B Minimal dependency health | `djconnect-api` replaces a manual npm dependency-risk review using its committed lockfile and active CI. | npm-only inventory; bounded support statement; canonical evidence, policy, differential, and public-CLI proof. | General supply-chain suite, Python/NuGet support, unsupported ecosystems, broad SBOM program. |
