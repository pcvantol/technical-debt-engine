...
class ReleaseBundleTests(unittest.TestCase):
    def test_complete_bundle_is_integrity_bound_and_tampering_fails(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            output = root / "bundle"
            command = ["python", "tools/assemble_release_bundle.py", "--candidate-sha", "a" * 40, "--release-version", PACKAGE_VERSION, "--output", str(output),
            ...
            self.assertEqual(PACKAGE_VERSION, verify(output)["bundle"]["bundleVersion"])
