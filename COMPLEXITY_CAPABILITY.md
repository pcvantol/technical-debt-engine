...
For supported Python symbols, canonical evidence contains cyclomatic repository, product-repository, language, file and symbol measurements: average, maximum and four-band distribution. High (`>=11`), very high (`>=21`) and critical (`>=41`) symbols emit deterministic findings that bind the measured value, threshold, affected entity and measurement evidence reference. Every symbol and finding is classified as `PRODUCT_SOURCE`, `TEST`, `FIXTURE` or `VERIFICATION`; classification never suppresses evidence. The bundled policy applies its hard complexity gate only to `PRODUCT_SOURCE`. A repository without supported symbols has an explicit `complexity.missing` finding and limitation rather than fabricated measurements.
