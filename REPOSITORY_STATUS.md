...
| Releases | `INTERNAL_RELEASE_PARTIALLY_COMPLETED`: tag `0.1.0`, GitHub Release, and Docker Hub OCI index are published from candidate `223ccfe4b3646f1907ee7e2d7a8c07e8989badd7`; PyPI is absent after the pinned PyPI Action container returned `manifest unknown`. |
...
| Post-merge lifecycle | PR #81 is `MERGED_RECONCILED` at `84478b5`; its immutable R1-4F archive exists. R1-4G records publication run `29529932503` and its partial outcome. |
...
| Internal Release 0.1.0 | `INTERNAL_RELEASE_PARTIALLY_COMPLETED`: Git tag, GitHub Release and Docker Hub `0.1.0` exist; PyPI and workflow publication evidence are blocked by unavailable pinned action container. |
