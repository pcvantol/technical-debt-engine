...
| Releases | `PYPI_PUBLICATION_BLOCKED`: PR #83 repaired the unavailable PyPI Action pin, but completion run `29531471511` correctly stopped before PyPI when the existing immutable tag push was rejected. Tag, GitHub Release, and Docker Hub OCI index remain published; PyPI is absent. |
...
| Post-merge lifecycle | PR #82 is `MERGED_RECONCILED` at `55673da`; its immutable R1-4G archive exists. PR #83 repairs PyPI Action availability; R1-4H records the resumability blocker. |
...
| Internal Release 0.1.0 | `PYPI_PUBLICATION_BLOCKED`: Git tag, GitHub Release and Docker Hub `0.1.0` exist; repaired action availability is verified, but current workflow rejects the already immutable tag before reaching PyPI. |
...
P1-9 operationalizes Trusted Delivery. `tde trusted-delivery` validates a clean candidate SHA, repository and branch; consumes canonical Software Assurance without repeating its assurance rules; verifies a versioned JSON manifest against candidate and artifact checksums; validates assurance-backed reproducibility/provenance; and records SHA-256 workflow-source provenance with immutable-action and least-privilege checks. PR #57 was objectively merged into `main` at `9a612192cdf0777b7c7758d068a47339f8771fb8` on 2026-07-15 05:11:59 UTC. It creates no release, certification, tag, or publication. Dogfooding TDE's clean committed candidate returned `PASS_WITH_WARNINGS` only because this non-release increment intentionally supplied no external manifest or independent artifact directories.
