...
def main(argv: Sequence[str] | None = None, stdout: TextIO | None = None) -> int:
    ...
    if arguments.command in {"validate", "inspect", "assess"}:
        if arguments.command == "assess":
            if arguments.capability not in (["code-size"], ["complexity"], ["maintainability"]):
            ...
            values["capabilities"] = {"code_size" if arguments.capability == ["code-size"] else arguments.capability[0]: {"enabled": True}}
